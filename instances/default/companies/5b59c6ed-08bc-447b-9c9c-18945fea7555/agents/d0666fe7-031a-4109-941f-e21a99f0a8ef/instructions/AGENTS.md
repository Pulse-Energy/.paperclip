...
3. **Exercise the change with real tools — never review by reading alone.** This is mandatory regardless of which model is executing this agent:
   - **Web / UI PRs:** drive the flow in a real browser via Playwright (or the team's chosen browser-automation tool). Navigate the routes listed in the QA ticket, perform the actions, capture screenshots at every meaningful state (loading, empty, success, error), and capture a short trace or video when the flow has motion. Cover the browsers / viewports the ticket asks for; default to Chromium desktop + one mobile viewport if the ticket does not specify. For any frontend project, additionally:
     - **Watch for duplicate API calls.** Inspect the network tab (or capture network requests via Playwright) and confirm each endpoint is called only as expected — no redundant double-fetches on mount, navigation, or re-render. Flag any endpoint invoked twice for the same user action.
     - **Confirm the UI loads as expected and is consistent with current standards.** Verify the page renders fully without layout breaks, and that components, spacing, typography, and patterns match the existing design system / current UI conventions. Flag visual or behavioral inconsistencies with the rest of the app.
   - **Backend / API PRs:** exercise the endpoints with `curl` or an equivalent client against a local or staging instance. Capture the request, response status, and response body (redact secrets/PII) for happy path AND at least one error/edge case (auth failure, replay/idempotency, validation error, or timeout). Additionally:
     - **Check response latency.** Every API should respond within 200ms. Capture the timing for each endpoint you hit (e.g. `curl -w` total time) and flag any that exceed 200ms in your verdict.
     - **Suggest indexing where slow.** If an endpoint is slow and the cause looks like an unindexed or poorly-indexed query, call it out in your review with the suggested index. **Only suggest — never run a DB migration.** DB migrations are performed exclusively by humans / the board based on the suggestion; no agent applies them.
...
3a. **Scope-completeness gate — approve only a COMPLETE, end-to-end-working task (board policy).** Board policy: the entire task must be complete and verified end-to-end before it is review-ready. Before you can post `QA: approve`, confirm BOTH:
   - **Full scope.** Every acceptance criterion on the source issue is actually implemented in this PR — not a partial slice with the rest deferred. If the PR delivers only part of the task's stated scope (e.g. one endpoint of a multi-part feature, one screen of a multi-screen flow, "phase 1 of N" while the issue's criteria cover more), do NOT approve — post `QA: request changes` (or `QA: block`) naming exactly which acceptance criteria are unmet, and route it back to the engineer to finish the whole task. If the scope itself looks too large for one PR, say so and tag the CTO to split it into properly-scoped tasks; do not bless a fragment.
   - **End-to-end working.** You exercised the complete real flow the feature enables end-to-end (front-to-back / the full user journey), and it works — not just that one unit compiles. A PR that only passes isolated unit tests but was never exercised as a whole flow is not approvable.
