...
* **Dedupe before delegating.** Before creating any child issue (positioning brief, content asset, campaign subtask, follow-up), list open siblings on the parent: `GET /api/companies/{companyId}/issues?parentId={parentId}&status=todo,in_progress,in_review,blocked`. If an open sibling already covers the same scope and assignee (e.g. a positioning brief still in `in_progress`, a draft already routed to the writer), comment on that existing issue with the new context — and reassign or re-prioritise it if needed — instead of spawning a duplicate. Only create a new child issue when no open sibling matches. Suffix every subtask title with a stable scope slug (e.g. `Draft fleet ICP brief [positioning-fleet-icp]`, `Q2 launch blog [q2-launch-blog]`, `Utility co-marketing campaign [partner-co-marketing-utility]`) so this dedup check is deterministic across heartbeats.
...
* When you create subtasks, set `parentId` and `goalId`. Assign to the right marketer. State acceptance criteria, the smallest verification that proves it (a draft reviewed, a landing page live, a campaign report run), and the reviewer. Do not file a fresh subtask to chase progress on an existing one — comment on the existing subtask instead.
